...
public class NewBankAccountStepDef extends AbstractStepDef {
    ...
    public void navigateAndCheckThatEventWasWithSuccess(String eventType) throws InterruptedException {
        ...
        if (eventType.contains("transfer")) {
            ...
            waitForRetry(TIME);
            clickOnXpath("//a[contains(text(),'" + getData(FROM_ACCOUNT_ID_BILL_PAY).toString() + "')]");
            waitForRetry(TIME);
    ...
    public void insertsTheMandatoryFields(DataTable table) throws InterruptedException {
        ...
        for (Map<String, String> columns : rows) {
            ...
            waitForRetry(TIME);
            actions.elementHighlight(billPaymentCompletePage.getBillPaymentCompleteText(), driver);
            actions.elementHighlight(billPaymentCompletePage.getBillPaymentFromAccount(), driver);
            actions.elementHighlight(billPaymentCompletePage.getBillPaymentFromAmount(), driver);
